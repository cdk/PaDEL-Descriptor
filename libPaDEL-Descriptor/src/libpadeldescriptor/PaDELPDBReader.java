 ...
 * @cdk.bug     1794439
 */
public class PaDELPDBReader extends DefaultChemObjectReader {

	private static ILoggingTool logger =
	    LoggingToolFactory.createLoggingTool(PaDELPDBReader.class);
	 ...
	 *
	 */
	public PaDELPDBReader(InputStream oIn) {
	 ...
	 *
	 */
	public PaDELPDBReader(Reader oIn) {
				...
				if (cRead != null) {
					...
					if ("SEQRES".equalsIgnoreCase(cCol)) {
						...
						logger.debug("Added new STRAND");
                                                ...
                                                // PaDEL
                                                try {
                                                    if(!createBondsWithRebondTool(molecularStructure))	{
                                                        ...
                                                        molecularStructure.removeAllBonds();
                                                    }
                                                } catch (Exception exception) {
...
//			 configure atoms
	...
	private PDBAtom readAtom(String cLine, int lineLength) {
		...
		if (lineLength < 59) {
//			throw new RuntimeException("PDBReader error during readAtom(): line too short"); // PaDEL
		}
